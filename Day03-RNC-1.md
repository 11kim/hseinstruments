...
**-2**. Еще раз проверим, что структура папок в репозитории вот <a href="https://olesar.github.io/KILI/img/KILI_RepositoryFolders.png" target="_blank">такая</a>. См. домашнее задание по github.
Пожалуйста, точно соблюдайте указания насчет имен папок и файлов, иначе робот, проверяющий работы, не найдет ваш файл или поставит оценку кому-нибудь другому. 

**-1**. Заодно сегодня мы познакомимся с форматом разметки файлов **markdown**. Это продвинутая версия вики-разметки, которая позволяет вставлять в веб-страницы фрагменты исполняемых скриптов. Впрочем, сегодня мы будем всего лишь редактировать ее текст в github.
Скопируйте файл по <a href="https://github.com/olesar/hseinstruments/blob/master/Data/%D0%A4%D0%B0%D0%BC%D0%B8%D0%BB%D0%B8%D1%8F.md">ссылке</a> к себе в папку `Day03_class`. Переименуйте файл так, чтобы названием файла была ваша фамилия (кириллицей, с заглавной буквы), а расширение оставалось `.md`. Убедившись, что файл оказался в вашем удаленном репозитории, воспользуйтесь кнопкой для его редактирования онлайн.
<img src="https://github.com/olesar/olesar.github.io/blob/master/KILI/img/Gihub_EditFile.png" width="300" /><br />  
...
# 1.3 Почему количество документов меньше количества вхождений? Ответ: Потому что слово достаточно частое и встречается в текстах более 1 раза. 
```
...
```
# 3.4 Ответ: ' ' обозначает &, т.е. запрос `баба с возу` означает слово, которое является какой-то формой слова 'баба', выглядящей как 'с' и 'возу'
```
